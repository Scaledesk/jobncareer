		...
		public function settings_field( array $options = array() ) {
			...
			if ( isset( $options['id'] ) && $options['id'] == 'Authorize Me' ) {
				...
				if( $lioauth_options ) {
					...
					if( $dDiff > 0 ) {
						$authorization_message = '<p style=" display: inline-block; margin-left: 10px; "> Authorization token expires in ' . $times['days'] . ' days, ' . $times['hours'] . ' hours ( <i>' . $date->format('m / d / Y') . '</i> ) </p>';
					} else {
		...
		function  settings_page () {
			...
			if ( isset($_GET['code']) ) {
			    ...
			    if(isset($token["access_token"]) && '' != $token["access_token"] && 5 < strlen( $token["access_token"] ) ) {
			        ...
			        echo '<div class="updated error"><p>Did not recieve an access token!</p></div>'; 
	    ...
	    if( $dDiff > 0 ) {
			...
			if( is_array($array_updates) && !empty($array_updates) ) {
				foreach ($array_updates as $update) {
					...
					$company_updates .= 		'<a target="_blank" href="' . $update_url . '">View on LinkedIn</a>';
					$company_updates .= 	'</span>';
					$company_updates .= 	'<div>';
					$company_updates .= 		$img;
				...
				$company_updates .= '<li>Sorry, no posts were received from LinkedIn!</li>';
			}
			$company_updates .= '</ul>';
